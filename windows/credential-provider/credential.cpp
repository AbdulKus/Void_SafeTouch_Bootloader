...
HRESULT PackLogon(const std::wstring& domain,const std::wstring& user,wchar_t* password,CREDENTIAL_PROVIDER_USAGE_SCENARIO scenario,BYTE** bytes,DWORD* size){
    ...
    if(scenario==CPUS_UNLOCK_WORKSTATION){DWORD required=0;CRED_PROTECTION_TYPE protectionType=CredUnprotected;if(!CredProtectW(FALSE,password,static_cast<DWORD>(wcslen(password)+1),nullptr,&required,&protectionType)&&GetLastError()!=ERROR_INSUFFICIENT_BUFFER)return HRESULT_FROM_WIN32(GetLastError());protectedPassword.resize(required);DWORD capacity=required;if(!CredProtectW(FALSE,password,static_cast<DWORD>(wcslen(password)+1),protectedPassword.data(),&capacity,&protectionType))return HRESULT_FROM_WIN32(GetLastError());effective=protectedPassword.data();}
...
HRESULT SafeTouchCredential::Advise(ICredentialProviderCredentialEvents* events){std::lock_guard lock(mutex_);if(events_)events_->Release();events_=events;if(events_)events_->AddRef();return S_OK;}
HRESULT SafeTouchCredential::UnAdvise(){std::lock_guard lock(mutex_);if(events_){events_->Release();events_=nullptr;}return S_OK;}
HRESULT SafeTouchCredential::SetSelected(BOOL* autoLogon){if(!autoLogon)return E_POINTER;Start();*autoLogon=Ready();return S_OK;}
HRESULT SafeTouchCredential::SetDeselected(){return S_OK;}
...
HRESULT SafeTouchCredential::GetStringValue(DWORD id,PWSTR* value){if(!value)return E_POINTER;if(id==FIELD_TITLE)return Duplicate(L"SafeVoid",value);if(id==FIELD_RETRY)return Duplicate(L"Retry",value);if(id==FIELD_STATUS){std::lock_guard lock(mutex_);return Duplicate(status_.c_str(),value);}return E_INVALIDARG;}
...
void SafeTouchCredential::Start(){if(running_.exchange(true))return;if(worker_.joinable())worker_.join();stop_=false;ready_=false;wantsDefault_=false;SecureZeroMemory(wrapKey_.data(),wrapKey_.size());if(!loadError_.empty()){SetStatus(loadError_.c_str());running_=false;return;}auto* owner=provider_;owner->AddRef();worker_=std::thread([this,owner]{Worker();owner->Release();});}
void SafeTouchCredential::Stop(){stop_=true;if(worker_.joinable()){if(worker_.get_id()==std::this_thread::get_id())worker_.detach();else worker_.join();}running_=false;ready_=false;wantsDefault_=false;SecureZeroMemory(wrapKey_.data(),wrapKey_.size());}
void SafeTouchCredential::StartMonitoring(){Start();}
void SafeTouchCredential::StopMonitoring(){Stop();}
void SafeTouchCredential::Worker(){using namespace safetouch;ComApartment apartment;HidDevice device;std::wstring error;SetStatus(L"Connect SafeVoid or insert card");while(!stop_&&!device.Open(error))Sleep(500);if(stop_){running_=false;return;}std::array<uint8_t,64> request{},reply{};request[0]=ST_CMD_INFO;if(!device.Command(request,reply,3000,error)||reply[1]!=ST_RESULT_OK||!ConstantTimeEqual(record_.deviceId,std::span<const uint8_t>(reply.data()+ST_INFO_DEVICE_ID_OFFSET,16))||!ConstantTimeEqual(record_.cardId,std::span<const uint8_t>(reply.data()+ST_INFO_CARD_ID_OFFSET,16))){SetStatus(L"Wrong or unregistered SafeVoid");running_=false;return;}
    Key32 nonce{};if(!RandomBytes(nonce)){SetStatus(L"Random generator failed");running_=false;return;}request.fill(0);request[0]=ST_CMD_AUTH_BEGIN;std::copy(nonce.begin(),nonce.end(),request.begin()+ST_AUTH_NONCE_OFFSET);if(!device.Command(request,reply,3000,error)||reply[1]!=ST_RESULT_OK){SecureZeroMemory(nonce.data(),nonce.size());SetStatus(L"SafeVoid rejected challenge");running_=false;return;}uint8_t previous=0xFF;
    while(!stop_){Sleep(250);request.fill(0);request[0]=ST_CMD_STATUS;if(!device.Command(request,reply,3000,error)){SetStatus(L"SafeVoid disconnected");break;}uint8_t state=reply[ST_REPLY_STATE_OFFSET];if(state!=previous){previous=state;if(state==ST_STATE_INSERT_CARD)SetStatus(L"Insert card");else if(state==ST_STATE_READING_CARD)SetStatus(L"Reading card...");else if(state==ST_STATE_PRESS_GREEN)SetStatus(L"Press GREEN on SafeVoid");else if(state==ST_STATE_ACCESS_DENIED)SetStatus(L"Access denied - remove card");else if(state==ST_STATE_CANCELED)SetStatus(L"Canceled");else if(state==ST_STATE_ERROR)SetStatus(L"SafeVoid error");if((state==ST_STATE_READING_CARD||state==ST_STATE_PRESS_GREEN)&&!wantsDefault_.exchange(true))provider_->CredentialBecameActive();}
        if(state==ST_STATE_CANCELED||state==ST_STATE_ERROR)break;if(state==ST_STATE_AUTH_OK){Key32 auth{};std::array<uint8_t,16> expected{};if(!UnprotectAuthKey(record_,auth)||!ComputeAuthProof(auth,nonce,record_.cardId,record_.deviceId,expected)||!ConstantTimeEqual(expected,std::span<const uint8_t>(reply.data()+ST_AUTH_PROOF_OFFSET,16))){SecureZeroMemory(auth.data(),auth.size());SetStatus(L"Cryptographic verification failed");break;}SecureZeroMemory(auth.data(),auth.size());{std::lock_guard lock(mutex_);std::copy_n(reply.begin()+ST_AUTH_WRAP_KEY_OFFSET,32,wrapKey_.begin());}SecureZeroMemory(reply.data()+ST_AUTH_WRAP_KEY_OFFSET,32);SecureZeroMemory(nonce.data(),nonce.size());ready_=true;SetStatus(L"Signing in...");running_=false;provider_->AuthenticationReady();return;}}
