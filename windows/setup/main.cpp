...
int wmain(int argc,wchar_t** argv){
    bool allowAtr=false;std::wstring requested;for(int i=1;i<argc;++i){if(std::wstring_view(argv[i])==L"--allow-atr")allowAtr=true;else if(std::wstring_view(argv[i])==L"--user"&&i+1<argc)requested=argv[++i];else{std::wcerr<<L"Usage: SafeTouchSetup [--user COMPUTER\\name] [--allow-atr]\n";return 2;}}
